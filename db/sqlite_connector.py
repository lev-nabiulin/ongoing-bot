...
def get_user_subs_formated(user_id):
    ...
    rows = cursor.execute("""
        ...
        ||'Всего серий: '|| ifnull(t.ep_total,'?_?')||', ты посмотрел: '|| ifnull(t.ep_now,'?_?')||char(10)
        ||'Время проверки: '|| ifnull(strftime('%d.%m.%Y %H:%M:%S',datetime(t.last_check_date, 'unixepoch')),'Не проверялось') name 
        FROM subscriptions s, titles t WHERE t.id = s.title_id and s.user_id=?""", 
        [user_id]).fetchall()
    cursor.close()
    if rows:
        result = []
